...
public final class LicenseManager {
    ...
    public static Result validate(Context ctx, String rawKey, boolean remember) {
        ...
        try {
            ...
            JSONArray results = com.replayx.receiver.util.Fs.query("keys", "keyString", key, 2);
            if (results.length() == 0 || !results.getJSONObject(0).has("document")) {
                if (com.replayx.receiver.util.Fs.lastQueryNetworkError()) {
                    out.networkError = true;
                    out.message = com.replayx.receiver.util.Fs.lastQueryDiagnostic("consulta da key");
                } else {
            ...
            String docId = com.replayx.receiver.util.Fs.docIdFromName(doc.getString("name"));
            JSONObject fields = doc.optJSONObject("fields");
            if (fields == null) fields = new JSONObject();

            String status = com.replayx.receiver.util.Fs.getStr(fields, "status", "");
            if (!"active".equals(status)) {
                out.message = "paused".equals(status) ? "Key pausada" : "Key inativa";
                return out;
            }

            int days = safeInt(com.replayx.receiver.util.Fs.getLong(fields, "days", 0L));
            int minutes = safeInt(com.replayx.receiver.util.Fs.getLong(fields, "minutes", 0L));
            long now = System.currentTimeMillis() / 1000L;
            Long first = com.replayx.receiver.util.Fs.getTsSec(fields, "firstUsed");
            Long paused = com.replayx.receiver.util.Fs.getTsSec(fields, "pausedAt");
            ...
            String legacyDevice = com.replayx.receiver.util.Fs.getStr(fields, "deviceId", "");
            String slot1 = com.replayx.receiver.util.Fs.getStr(fields, "slot1DeviceId", "");
            String slot2 = com.replayx.receiver.util.Fs.getStr(fields, "slot2DeviceId", "");
            String slot1Type = com.replayx.receiver.util.Fs.getStr(fields, "slot1Type", "");
            String slot1Model = com.replayx.receiver.util.Fs.getStr(fields, "slot1Model", "");
            ...
            int storedCount = safeInt(com.replayx.receiver.util.Fs.getLong(fields, "devicesUsed", 0L));
            ...
            if (slot1.isEmpty() && !legacyDevice.isEmpty()) {
                ...
                slot1Model = slot1Model.isEmpty()
                        ? com.replayx.receiver.util.Fs.getStr(fields, "deviceModel", "") : slot1Model;
                ...
                putPatch(migration, migrationMask, "slot1UsedAt", com.replayx.receiver.util.Fs.ts(firstSec));
                ...
                if (!persistSlotRegistration(docId, migration, migrationMask.toString(), slot1, 1)) {
                    out.message = com.replayx.receiver.util.Fs.lastPatchDiagnostic("migração do dispositivo");
                    return out;
                }
                count = 1;
            }
            ...
            if (!slot1.isEmpty() && slot1.equals(myDevice) || !slot2.isEmpty() && slot2.equals(myDevice)) {
                ...
                if (count >= 2) {
                    ...
                    putPatch(patch, mask, "slot1Type", "receiver");
                    putPatch(patch, mask, "slot1Model", Build.MANUFACTURER + " " + Build.MODEL);
                    putPatch(patch, mask, "slot1UsedAt", com.replayx.receiver.util.Fs.ts(now));
                    if (first == null) putPatch(patch, mask, "firstUsed", com.replayx.receiver.util.Fs.ts(now));
                    slot1 = myDevice;
                    count = Math.max(count, 1);
                } else if (slot2.isEmpty()) {
                    putPatch(patch, mask, "slot2DeviceId", myDevice);
                    putPatch(patch, mask, "slot2Type", "receiver");
                    putPatch(patch, mask, "slot2Model", Build.MANUFACTURER + " " + Build.MODEL);
                    putPatch(patch, mask, "slot2UsedAt", com.replayx.receiver.util.Fs.ts(now));
                    if (first == null) putPatch(patch, mask, "firstUsed", com.replayx.receiver.util.Fs.ts(now));
                    slot2 = myDevice;
                    count = 2;
                } else {
                ...
                if (!persistSlotRegistration(docId, patch, mask.toString(), myDevice, count)) {
                    out.message = com.replayx.receiver.util.Fs.lastPatchDiagnostic("registro do dispositivo");
                    return out;
                }
            }
            ...
            out.key = key;
            out.user = com.replayx.receiver.util.Fs.getStr(fields, "user", "");
            out.status = status;
    ...
    private static boolean persistSlotRegistration(String docId, JSONObject patch, String mask,
        ...
        boolean ok = com.replayx.receiver.util.Fs.patchDoc(path, patch, mask, "");
        if (!ok) return false;

        JSONObject after = com.replayx.receiver.util.Fs.getDoc(path);
        ...
        boolean visible = (expectedDevice.equals(com.replayx.receiver.util.Fs.getStr(after, "slot1DeviceId", ""))
                    || expectedDevice.equals(com.replayx.receiver.util.Fs.getStr(after, "slot2DeviceId", "")))
                && com.replayx.receiver.util.Fs.getLong(after, "devicesUsed", -1L) >= expectedCount;
        if (!visible) {
            ok = com.replayx.receiver.util.Fs.patchDoc(path, patch, mask, "");
            if (!ok) return false;
            after = com.replayx.receiver.util.Fs.getDoc(path);
            if (after == null) return true;
            visible = (expectedDevice.equals(com.replayx.receiver.util.Fs.getStr(after, "slot1DeviceId", ""))
                        || expectedDevice.equals(com.replayx.receiver.util.Fs.getStr(after, "slot2DeviceId", "")))
                    && com.replayx.receiver.util.Fs.getLong(after, "devicesUsed", -1L) >= expectedCount;
        }
        return visible;
    ...
    private static void putPatch(JSONObject patch, StringBuilder mask, String path, Object value) throws Exception {
        ...
        if (value instanceof JSONObject) {
            ...
            firestoreValue = com.replayx.receiver.util.Fs.nul();
        } else if (value instanceof Boolean) {
            firestoreValue = com.replayx.receiver.util.Fs.bool((Boolean) value);
        } else if (value instanceof Number) {
            firestoreValue = com.replayx.receiver.util.Fs.num(((Number) value).longValue());
        } else {
            firestoreValue = com.replayx.receiver.util.Fs.str(String.valueOf(value));
        }
